...
- Create New Dispatch and Add Container dialogs stay open on backdrop clicks.
- ESC key does not close these dialogs.
- Dialogs close only via the header close icon, Cancel, or Save actions.
- Dialog content blocks outside-click dismissal for consistent data entry flow.
