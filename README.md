...
### 4. Image Storage Workflow

Image uploads for dispatch container photos use Firebase Storage. The app prefers a secondary Firebase project for storage when OneDelivery env vars are present to avoid billing-disabled issues on the primary project. If the OneDelivery config is missing, uploads fall back to the primary Storage bucket.

To ensure image uploads work in development:
- Provide a valid Storage bucket in the primary project env vars, or
- Configure the OneDelivery Storage project using the env vars below
...
### Dispatches Page Refresh (Feb 2026)
- **Create Dispatch Modal**: Fits viewport with reliable scrolling and reduced blank space.
- **Add Container Modal**: Compact layout, stable scroll area, 40ft default size, and booking date auto-close.
- **Form Simplification**: Removed Loading Date from container form and Date/Time from dispatch form.
- **Container List UI**: Containers shown in a clean table inside Create Dispatch.
- **All Dispatches Card**: Removed container detail cards to keep the list compact.
- **Photo Upload Resilience**: Dispatch save completes even if Storage upload is unauthorized.
