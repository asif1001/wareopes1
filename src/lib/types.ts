...
export type DispatchContainerPhoto = {
    fileName: string;
    storagePath: string;
    downloadURL: string;
...
export type DispatchContainerTransport = {
...
export type DispatchContainerPersonnel = {
    loaderName: string;
    checkerName: string;
};
...
export type DispatchContainer = {
    ...
    transport: DispatchContainerTransport;
    personnel: DispatchContainerPersonnel;
    photos: DispatchContainerPhoto[];
};
...
export type SerializableDispatchContainerPhoto = {
    fileName: string;
    storagePath: string;
    downloadURL: string;
    uploadedAt: string;
...
export type SerializableDispatchContainer = Omit<DispatchContainer, 'bookingDate' | 'photos'> & {
    bookingDate: string;
    photos: SerializableDispatchContainerPhoto[];
};

export type SerializableDispatch = Omit<Dispatch, 'dateTime' | 'loadingDate' | 'createdAt' | 'updatedAt' | 'containers'> & {
    dateTime: string;
    loadingDate?: string;
    createdAt: string;
    updatedAt: string;
    containers?: SerializableDispatchContainer[];
};
...
export type AppPageKey = 'shipments' | 'dispatches' | 'tasks' | 'settings' | 'production' | 'productivity' | 'maintenance' | 'licenses' | 'oil_status' | 'staff' | 'reports' | 'feedback'; // extendable
