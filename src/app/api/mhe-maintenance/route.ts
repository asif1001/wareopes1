...
async function resolveBucket() {
  ...
  if (!bucketName) throw new Error('No Storage bucket found for MHE Maintenance.');
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      payload.mheId = String(raw.mheId || '');
      ...
      payload.mheId = String(body.mheId || '');
    ...
    if (!payload.mheId || !payload.date || !payload.type || !payload.workDescription) {
      return NextResponse.json({ success: false, error: 'mheId, date, type, and workDescription are required' }, { status: 400 });
    }

    const id = nanoid();
    const now = admin.firestore.FieldValue.serverTimestamp();
    const nowTs = admin.firestore.Timestamp.now();
    const docRef = adb.collection('mhe_maintenance').doc(id);
    await docRef.set({
      mheId: payload.mheId,
      date: payload.date,
      type: payload.type,
    ...
    if (attachmentFile) {
      try {
        ...
        const storagePath = `mhe-maintenance/${id}/attachments/${Date.now()}-${nanoid()}-${safeName}`;
        ...
        console.warn('MHE Maintenance attachment upload failed:', (err as any)?.message || err);
      }
    }
    ...
    console.error('POST /api/mhe-maintenance error:', e);
    return NextResponse.json({ success: false, error: e?.message || 'Server error' }, { status: 500 });
  }
}